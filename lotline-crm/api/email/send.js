...
function buildEmailHtml(bodyHtml, bodyText, trackingPixelId) {
  ...
  if (bodyHtml) {
    // Inject pixel before closing body tag, or append it
 ...
 * Supports HTML body and CC recipients.
 */
...
export default async function handler(req, res) {
  ...
  // Generate tracking pixel ID for this email
  ...
  // Log to deal_emails if we have a dealId
  if (dealId && orgId && user) {
    try {
      ...
      await supa.from('activity_notes').insert({
        ...
        created_by:      user.id,
        note_type:       'email',
        body:            `📧 Email sent to ${toDisplay} — Subject: ${subject}`,
      });
    } catch (err) {
