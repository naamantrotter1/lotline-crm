...
module.exports = async function handler(req, res) {
  ...
  if (isSC || (qParno && !isNC)) {
    ...
    try {
      ...
      return res.json({
        ...
        siteAddr: null, acres: (p.Acreage > 0 ? p.Acreage : null) ?? (p.Shape_Area > 0 ? p.Shape_Area / 43560 : null),
        landVal: p.L_Value ?? null, bldgVal: null, totVal: p.M_Value ?? null,
        landUse: p.Land_Use?.trim() || null, zoning: p.Zoning?.trim() || null,
        saleYear: null, county: p.County || null, subdivision: null,
        state: 'SC', geometry: f.geometry || null,
      });
    } catch (err) {
  ...
  try {
    ...
    return res.json({
      parcelId: a.parno || a.altparno || null, owner: a.ownname?.trim() || null,
      mailAddr: [a.mailadd, a.mcity, a.mstate, a.mzip].filter(Boolean).join(', ') || null,
      siteAddr: [a.siteadd, a.scity].filter(Boolean).join(', ') || null,
      acres: a.gisacres ?? null, landVal: a.landval ?? null, bldgVal: a.improvval ?? null,
      totVal: a.parval ?? null, landUse: a.parusedesc?.trim() || null, saleYear,
      county: a.cntyname || null, subdivision: a.subdivisio?.trim() || null,
      state: 'NC', geometry,
    });
  } catch (err) {
    return res.status(502).json({ error: err.message });
