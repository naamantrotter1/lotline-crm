...
import { X, ChevronRight, Star, MapPin, Archive } from 'lucide-react';
    ...
    >
      <div className="flex justify-between items-start mb-1 gap-2">
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          {deal.county && <span className="text-xs text-gray-400 border border-gray-200 rounded px-1.5 py-0.5">{deal.county}</span>}
        </div>
      </div>
      {deal.tags && deal.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-1">
          {deal.tags.map(tag => <Tag key={tag} type={tag}>{tag}</Tag>)}
        </div>
      )}
      {deal.arv && (
      ...
      <div className="flex items-center justify-between mt-1">
        <div className="flex items-center gap-1">
          <span className={`text-sm font-bold ${(deal.netProfit || 0) >= 0 ? 'text-green-600' : 'text-red-500'}`}>
            {deal.netProfit !== undefined ? `$${Math.abs(deal.netProfit).toLocaleString()}` : 'N/A'}
          </span>
          ...
          return (
            <div key={stage} className="flex-shrink-0 w-72">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="bg-sidebar text-white text-xs font-bold px-2 py-0.5 rounded-full">{deals.length}</span>
              </div>
