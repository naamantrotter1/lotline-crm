...
import {
  ...
  Landmark, Handshake, XCircle, Info, Lock, LockOpen, Plus, Trash2,
} from 'lucide-react';
...
function FinancingScenarioPanel({
  ...
  originationRolled, setOriginationRolled,
  ...
  const hmLoanBasisForInterest = effectiveLoanAmount + (originationRolled ? originationFee : 0);
  const totalClosingCosts = (originationRolled ? 0 : originationFee)
    + (servicingFeeFlat || 0) + (drawFeeHm || 0) + (underwritingFee || 0)
    + (attorneyDocFee || 0) + (appraisalFeeHm || 0) + (legalFeeHm || 0);
  const monthlyInterestHm = hmLoanBasisForInterest * (interestRate / 100) / 12;
          ...
          <div className="bg-white rounded-xl border border-gray-100 px-4 py-3">
            ...
            <div className="grid grid-cols-2 gap-x-6">
                ...
                <div className="flex items-center gap-4 flex-wrap">
                  ...
                  {arvVal > 0 && (
                    <div className="text-xs text-gray-500">
                      Max loan: <span className="font-semibold text-sidebar">${Math.round(arvVal * (ltvCapPctHm ?? 60) / 100).toLocaleString()}</span>
                      <span className="text-gray-400 ml-1">(ARV ${arvVal.toLocaleString()} × {ltvCapPctHm ?? 60}%)</span>
                    </div>
                  )}
                </div>
                {arvVal > 0 && effectiveLoanAmount > 0 && (() => {
                  ...
                  return (
                    <div className={`mt-1.5 flex items-center gap-1.5 text-xs font-medium ${ok ? 'text-green-600' : 'text-red-500'}`}>
                      ...
                      {ok ? `Within LTV cap ($${Math.round(effectiveLoanAmount).toLocaleString()} = ${pct}% LTV)` : `Exceeds LTV cap ($${Math.round(effectiveLoanAmount).toLocaleString()} = ${pct}% LTV)`}
                    </div>
                  );
                })()}
              ...
              <div className="py-2 col-span-2">
                ...
                {capitalDeployedDate && !firstPaymentDate && !paymentDueDay && (() => {
                  ...
                  const autoDate = `${y}-${String(m).padStart(2,'0')}-01`;
                  return (
                    <div className="flex items-center gap-1.5 mt-1">
                      <Calendar size={11} className="text-accent flex-shrink-0" />
                      <p className="text-[11px] text-accent">
                        First IO payment due: <strong>{new Date(autoDate + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</strong>
          ...
          <div className="bg-white rounded-xl border border-gray-100 px-4 py-3">
            ...
            <div className="grid grid-cols-2 gap-x-6">
              ...
              <div className="py-2">
                ...
                {!readOnly && originationFee > 0 && (
                  <div className="mt-1.5 inline-flex rounded border border-gray-200 overflow-hidden text-[10px] font-semibold">
                    <button type="button" onClick={() => setOriginationRolled(false)} className={`px-2 py-0.5 transition-colors ${!originationRolled ? 'bg-accent text-white' : 'bg-white text-gray-400 hover:bg-gray-50'}`}>Cash at Closing</button>
                    <button type="button" onClick={() => setOriginationRolled(true)} className={`px-2 py-0.5 transition-colors ${originationRolled ? 'bg-accent text-white' : 'bg-white text-gray-400 hover:bg-gray-50'}`}>Rolled into Loan</button>
                  </div>
                )}
                {originationRolled && originationFee > 0 && (
              ...
              {showHmAdvanced && (
                <>
                  ...
                  <div className="py-2">
                    ...
                    <DecimalInput value={legalFeeHm || 0} onChange={setLegalFeeHm} className={iCls} />
...
function DealDetailContent({ deal }) {
  ...
  const [ltvCapPctHm, setLtvCapPctHm] = useState(sd.ltvCapPctHm ?? 60);
  const [originationRolled, setOriginationRolled] = useState(sd.originationRolled ?? false);
  const [legalFeeHm, setLegalFeeHm] = useState(sd.legalFeeHm ?? 0);
  ...
  useEffect(() => {
    ...
    const updatedDeal = {
      ...
      scenarioData: {
        ...
        drawFeeHm, underwritingFee, attorneyDocFee, appraisalFeeHm,
        ltvCapPctHm, originationRolled, legalFeeHm, cashSource,
    ...
    ltvCapPctHm, originationRolled, legalFeeHm,
    investorReturnType, investorAssignmentStatus,
      ...
      {/* Tab content */}
        ...
        {activeTab === 'financing' && financingTabEnabled && (
          <FinancingScenarioPanel
            ...
            ltvCapPctHm={ltvCapPctHm} setLtvCapPctHm={setLtvCapPctHm}
            originationRolled={originationRolled} setOriginationRolled={setOriginationRolled}
            legalFeeHm={legalFeeHm} setLegalFeeHm={setLegalFeeHm}
