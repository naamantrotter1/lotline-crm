  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">{docs.length} document{docs.length !== 1 ? 's' : ''} shared with you</p>
      </div>
      ...
      {/* Content */}
      {loading ? (
        <div className="space-y-3">{[...Array(3)].map((_, i) => <div key={i} className="bg-white/5 rounded-xl h-16 animate-pulse" />)}</div>
      ) : filtered.length === 0 ? (
        ...
        <div className="space-y-6">
          {Object.entries(byYear).sort((a, b) => b[0] - a[0]).map(([year, yearDocs]) => (
            <div key={year}>
              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-3">{year}</h3>
              <div className="bg-[#1c2130] rounded-xl border border-white/8 overflow-hidden divide-y divide-white/5">
                {yearDocs.map(doc => (
                  <div key={doc.id} className="flex items-center justify-between p-4 hover:bg-white/3 transition-colors group">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          {doc.deals?.address && (
                            <span className="text-[10px] text-gray-500 truncate">{doc.deals.address}</span>
                          )}
                          <span className="text-[10px] text-gray-600">{fmtDate(doc.created_at)}</span>
                          {doc.file_size_bytes && (
                            <span className="text-[10px] text-gray-600">{fmtBytes(doc.file_size_bytes)}</span>
                          )}
                        </div>
                      </div>
                    </div>
                    <a
                      ...
                      download
                      className="ml-3 flex-shrink-0 flex items-center gap-1.5 text-xs text-gray-400 hover:text-accent transition-colors px-3 py-1.5 rounded-lg hover:bg-white/5"
