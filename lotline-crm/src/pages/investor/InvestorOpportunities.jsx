...
function ReserveModal({ deal, investor, onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className="bg-white dark:bg-[#1c2130] rounded-2xl border border-gray-200 dark:border-white/8 w-full max-w-md p-6 shadow-2xl"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-start justify-between mb-5">
          <div>
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">Reserve Interest</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate max-w-[280px]">{deal.address}</p>
          </div>
          <button onClick={onClose} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white ml-3">
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1.5">Investment Amount</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-7 pr-4 py-2.5 bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-600 outline-none focus:border-accent/50"
                required
              />
            </div>
            {deal.min_check_size && (
              <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-1">Minimum: {fmt(deal.min_check_size)}</p>
            )}
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2.5 bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-600 outline-none focus:border-accent/50 resize-none"
            />
          </div>
          ...
          <div className="flex gap-3 pt-1">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-2.5 rounded-lg border border-gray-200 dark:border-white/8 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
            >
              Cancel
            </button>
            <button
...
function DealCard({ deal, investor, myInterestIds }) {
  ...
  const stageColor = STAGE_COLORS[deal.stage] ?? 'bg-gray-200 text-gray-500';
      ...
      <div className="bg-white dark:bg-[#1c2130] rounded-xl border border-gray-200 dark:border-white/8 overflow-hidden">
        ...
        <div className="p-5">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap mb-1">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white truncate">{deal.address}</h3>
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${stageColor}`}>{deal.stage}</span>
              </div>
              {deal.county && (
                <p className="text-xs text-gray-500 dark:text-gray-400">{deal.county}, {deal.state}</p>
              )}
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
            {[
              { label: 'ARV',            value: fmt(deal.arv),           color: 'text-gray-900 dark:text-white'       },
          ...
          {pctFilled !== null && (
            <div className="mt-4">
              ...
              {alloc != null && (
                <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-1">{fmt(alloc)} remaining</p>
              )}
            </div>
          )}
        </div>
        ...
        {/* Expandable proforma */}
        {expanded && (
          <div className="border-t border-gray-200 dark:border-white/8 px-5 py-4 bg-gray-50 dark:bg-[#0f1117] space-y-3">
            <p className="text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-widest font-semibold mb-2">Proforma</p>
            <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs">
              {[
                ...
                <div key={label} className="flex justify-between">
                  <span className={bold ? 'text-gray-700 dark:text-gray-300 font-semibold' : 'text-gray-500 dark:text-gray-400'}>{label}</span>
                  <span className={bold ? (green ? 'text-green-400 font-semibold' : 'text-gray-900 dark:text-white font-semibold') : 'text-gray-700 dark:text-gray-300'}>{value}</span>
                </div>
              ))}
            </div>

            {deal.financing && (
              <div className="pt-2 border-t border-gray-200 dark:border-white/8 flex justify-between text-xs">
                <span className="text-gray-500 dark:text-gray-400">Financing</span>
                <span className="text-gray-700 dark:text-gray-300">{deal.financing}</span>
              </div>
            )}
            {deal.utility_scenario && (
              <div className="flex justify-between text-xs">
                <span className="text-gray-500 dark:text-gray-400">Utilities</span>
                <span className="text-gray-700 dark:text-gray-300">{deal.utility_scenario}</span>
              </div>
            )}
            {deal.home_model && (
              <div className="flex justify-between text-xs">
                <span className="text-gray-500 dark:text-gray-400">Home Model</span>
                <span className="text-gray-700 dark:text-gray-300">{deal.home_model}</span>
              </div>
            )}
          </div>
        )}
    ...
    <div className="p-6 md:p-8 max-w-4xl mx-auto space-y-6">
      ...
      {!loading && deals.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={label} className="bg-white dark:bg-[#1c2130] rounded-xl p-4 border border-gray-200 dark:border-white/8">
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="bg-gray-100 dark:bg-white/8 rounded-xl h-40 animate-pulse" />
          ))}
        </div>
      ) : deals.length === 0 ? (
        <div className="bg-white dark:bg-[#1c2130] rounded-xl p-12 text-center border border-gray-200 dark:border-white/8">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm">No opportunities available right now.</p>
          <p className="text-gray-400 dark:text-gray-500 text-xs mt-1">Check back soon as new deals come online.</p>
        </div>
      ) : (
