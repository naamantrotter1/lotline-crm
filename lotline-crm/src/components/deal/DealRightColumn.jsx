...
// ── Main component ────────────────────────────────────────────────────────────
const DOC_CATEGORIES = ['Construction Permit', 'Contract', 'Deed', 'Environmental Permit', 'Financing', 'GIS Map', 'Inspection', 'Manufactured Home Quote', 'Order Forms', 'Photos', 'Plat Map', 'Soil Report', 'Survey', 'Title Policy', 'Title Report', 'Zoning Verification', 'Other'];
...
export default function DealRightColumn({ deal, readOnly, onCreateTask }) {
  ...
  const handleDocUpload = async (e) => {
    ...
    if (!storageErr) {
      ...
      // Log to activity feed — body is just the category. The activity
      // renderer derives the title ("<Category> uploaded") from it. The
      // file itself is downloaded from the Documents card, so we don't
      // embed the URL here (which previously broke markdown-link rendering
      // whenever Supabase storage URLs contained parens or spaces).
